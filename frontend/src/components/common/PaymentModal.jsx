...
const PaymentModal = ({ visible, onClose, bookingDetails, onPaymentSuccess }) => {
  ...
  const navigate = useNavigate(); // Assuming you have react-router for navigation
  const handlePayment = async () => {
    setProcessing(true);
    console.log("Processing VNPay payment for:", bookingDetails);
    const data = {
      showtimeId: bookingDetails.showtimeId,
      seats: bookingDetails.seats[0],
      totalAmount: bookingDetails.totalAmount,
      paymentMethod: "BANK"
    }
    try {
      // --- Real VNPay Integration Logic Here ---
      const res = await axios.post('http://localhost:8080/api/guest/booking', data, { withCredentials: true }); // Ensure credentials are sent if needed
  ...
  const { movieTitle = 'N/A', seats = [], totalAmount = 0, time = dayjs().format('HH:mm:ss'), date = dayjs().format('YYYY-MM-dd'), room = 'N/A' } = bookingDetails;

...
const Payment = () => {
    ...
    const fetchData = async (page = 0, size = 5) => {
        try {
            ...
            setState((prev) => ({
                ...
                pagination: {
                    ...prev.pagination,
                    totalElements: bookingsRes.data.totalElements,
                },
            }));
            messageApi.destroy();
    ...
    useEffect(() => {
        ...
        setState((prev) => ({ ...prev, filteredBookings: result }));
    }, [state.filters, state.bookings]);
    ...
    const handleSubmit = async (action, values = null) => {
        const config = {
            ...
            edit: {
                method: "put",
                url: `http://localhost:8080/api/bookings/${state.selectedPayment?.id}`,
                data: values,
            },
            delete: {
                method: "delete",
    ...
    const { filteredBookings, filters, modals, selectedPayment } = state;

...
const Seat = () => {
  ...
  const [filteredData, setFilteredData] = useState(
    initData.filter((item) => item.cinema === cinemas[0].label)
  );
  ...
  const handleCinemaSelect = ({ key }) => {
    const selected =
      cinemas.find((c) => c.key === key)?.label || cinemas[0].label;
    setSelectedCinema(selected);
    setSelectedRoom(null);
    setFilteredData(initData.filter((item) => item.cinema === selected));
  };
  ...
  const handleRoomSelect = ({ key }) => {
    ...
    setFilteredData(
      initData.filter(
        (item) => item.cinema === selectedCinema && item.room === room
      )
    );
  };
  ...
  return (
    <Card
      ...
      extra={
        <Space>
          <Dropdown menu={{ items: cinemaItems }} trigger={["click"]}>
            <Button>
              <Space>
                {selectedCinema}
                <DownOutlined />

...
public class ScreenService implements IService<Screen, String> {
    ...
    @Autowired
    public ScreenService(ScreenRepository screenRepository, AutoGeneratedCode codeGenerator, MongoTemplate mongoTemplate, SeatService seatService) {
    ...
    public Map<String, String> findAllNamesWithID() {
        return screenRepository.findAll().stream().filter(screen -> screen.getScreenCode() != null && screen.getCinemaCode() != null).collect(Collectors.toMap(Screen::getCinemaCode, Screen::getScreenCode, (existing, replacement) -> existing));
    }

    public Map<String, List<String>> findAllNamesWithIDs() {
        return screenRepository.findAll().stream().filter(screen -> screen.getScreenCode() != null && screen.getCinemaCode() != null).collect(Collectors.groupingBy(Screen::getCinemaCode, Collectors.mapping(Screen::getScreenCode, Collectors.toList())));
    }
    ...
    public List<Screen.ScreenType> findAllTypes() {
        return screenRepository.findAllByType().stream().map(Screen::getType).distinct().collect(Collectors.toList()).reversed();
    }
